...
var floor = mapMaximumY + 2;
var sandEntryPoint = new MapNode(500, 0);

var sandHasBlockedEntry = false;
while(!sandHasBlockedEntry)
{
    ...
    while (!itemHasComeToRest)
    {
        ...
        if (map.ContainsKey(sandEntryPoint))
        {
            sandHasBlockedEntry = true;
            itemHasComeToRest = true;
        }
    }
}
...
Console.WriteLine($"Day 14 - Part 2: {map.Count(node => node.Value == NodeType.Sand)}");
